...
Vercel Hobby 플랜에 배포한다. 광고·수익화가 없는 개인 블로그라 약관상 문제가 없다 (`docs/PRD.md` 비-목표).
...
> Vercel 플랜은 **프로젝트가 아니라 팀 단위**다. 같은 팀의 다른 프로젝트가 상업적으로 쓰이면
> (광고·결제·후원 — Vercel 공정이용 기준) 팀 전체가 Pro 가 필요해진다. 블로그 자체는 광고가 없어
> non-commercial 이다.
